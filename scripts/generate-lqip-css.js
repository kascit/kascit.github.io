...
function main() {
  ...
  if (!fs.existsSync(manifestPath)) {
    console.error(`ERROR: responsive-manifest.json not found at ${manifestPath}`);
    process.exit(1);
  }
  ...
  const lines = [
    "/* Auto-generated LQIP classes. Do not edit by hand. */",
  ];
  ...
  for (const [key, data] of Object.entries(manifest)) {
    ...
    lines.push(`.${className}{background-image:url(\"${lqip}\");background-size:cover;background-position:center;}`);
    count += 1;
  }
  ...
  console.log(`Generated ${count} LQIP class(es) in ${path.relative(ROOT, cssPath).replace(/\\/g, "/")}.`);
}
