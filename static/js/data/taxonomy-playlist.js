...
function setNavSlot(slotSelector, direction, href, subtitle, title, onClick) {
  ...
  document.dispatchEvent(
    new CustomEvent("tooltips:update", { detail: { element: anchor } }),
  );
}
...
function tryOverrideBlogNav(playlist) {
  ...
  if (prevItem) {
    ...
    setNavSlot(".prev-nav-item", "prev", exitPath, "Playlist", "Exit Playlist");
  }
  ...
  if (nextItem) {
    ...
    setNavSlot(".next-nav-item", "next", exitPath, "Playlist", "Exit Playlist");
  }
}
...
export function initTaxonomyPlaylist() {
  ...
  const postNodes = Array.from(
    document.querySelectorAll("[data-taxonomy-playlist-post]"),
  );
  const params = new URLSearchParams(window.location.search);
  ...
  const openFirstBtn = root.querySelector(
    "[data-taxonomy-playlist-open-first]",
  );
  ...
  function syncPostState() {
    postNodes.forEach((node) => {
      ...
      btn.setAttribute(
        "title",
        selected ? "Remove from playlist" : "Add to playlist",
      );
    });
  }
  ...
  function addPost(path, title) {
    ...
    playlist.push({
      ...
      title: normalizeTitle(title || path),
    });
  }
  ...
  listNode.addEventListener("drop", (event) => {
    ...
    const targetUrl = normalizePath(
      targetRow.getAttribute("data-playlist-url"),
    );
  ...
  openFirstBtn.addEventListener("click", () => {
    if (playlist.length === 0) return;
    window.location.href = buildPlaylistUrl(
      playlist[0].url,
      playlist,
      exitPath,
    );
