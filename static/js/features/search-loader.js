...
function getSearchMount() {
  return (
    document.querySelector("[data-search-mount]") ||
...
function getSearchModal() {
  ...
  return (
    (mount && mount.querySelector("[data-search-modal]")) ||
...
function getSearchInput() {
  var mount = getSearchMount();
  return (
    (mount && mount.querySelector("[data-search-input]")) ||
    ...
    while (result) {
      ...
      if (start > cursor) {
        fragment.appendChild(
          document.createTextNode(source.slice(cursor, start)),
        );
      }
...
function updatePageFindCount() {
  ...
  state.count.textContent =
    String(state.activeIndex + 1) + " / " + String(state.matches.length);
}
...
function setActivePageFindMatch(nextIndex, shouldScroll) {
  ...
  if (shouldScroll && state.matches[normalized]) {
    state.matches[normalized].scrollIntoView({
      block: "center",
      inline: "nearest",
      behavior: "smooth",
    });
...
function formatSearchResultItem(item, terms) {
  ...
  link.className =
    "search-result-link block px-4 py-3 rounded-md transition-colors duration-150";
  ...
  title.className =
    "search-result-title font-semibold text-sm text-base-content mb-1";
  renderHighlightedText(
    title,
    item && item.item && typeof item.item.title === "string"
      ? item.item.title
      : "Untitled",
    terms,
  );
  ...
  excerpt.className =
    "search-result-excerpt text-xs text-base-content/60 line-clamp-2";
  renderTeaserText(
    excerpt,
    makeTeaser(item && item.item ? item.item.body : "", terms),
  );

  var arrow = document.createElement("div");
  arrow.className =
    "search-result-arrow flex-shrink-0 opacity-0 transition-opacity duration-150";
...
function setSearchHeader($searchResultsHeader, term, count, isEmpty) {
  ...
  wrap.appendChild(
    document.createTextNode(
      String(count) + " result" + (count === 1 ? "" : "s") + " for ",
    ),
  );
...
function initSearch() {
  ...
  var $searchResultsContainer =
    (mount && mount.querySelector("[data-search-results-container]")) ||
    document.querySelector(".search-results-container");
  var $searchResultsHeader =
    (mount && mount.querySelector("[data-search-results-header]")) ||
    document.querySelector(".search-results__header");
  var $searchResultsItems =
    (mount && mount.querySelector("[data-search-results-items]")) ||
    document.querySelector(".search-results__items");
  var $searchClearButton =
    (mount && mount.querySelector("[data-search-clear]")) ||
    document.querySelector("[data-search-clear]");
  if (!$searchResultsContainer || !$searchResultsHeader || !$searchResultsItems)
    return;
  var MAX_ITEMS = 10;
  var selectedIndex = -1;
  ...
  function appendAskAiResult(term) {
    ...
    $searchResultsItems.appendChild(
      formatAskAiResultItem(term, askAiConfig.label),
    );
  }
  ...
  function executeAskAi(term) {
    ...
    askAiAboutQuery(term, { source: "search-modal" }).catch(function (error) {
      ...
      if (code === "AUTH_REQUIRED") {
        $searchResultsHeader.textContent =
          askAiConfig.guestMessage || "Sign in to use Ask AI";
  ...
  var modalBackdrop =
    (mount && mount.querySelector("[data-search-backdrop]")) ||
    mount ||
    document.querySelector(".modal");
  var $searchCloseButton =
    (mount && mount.querySelector("[data-search-close]")) ||
    document.querySelector("[data-search-close]");
  ...
  function closeSearchModal(options) {
    ...
    var target =
      lastModalTrigger && lastModalTrigger.isConnected
        ? lastModalTrigger
        : fallbackTarget;
  ...
  $searchInput.addEventListener("keydown", function (e) {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (link) {
        var action = link.getAttribute("data-search-action");
        var term =
          link.getAttribute("data-search-query") || getCurrentSearchTerm();
