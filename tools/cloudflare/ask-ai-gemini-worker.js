...
function resolveCorsOrigin(request, env) {
  const requestOrigin = sanitizeString(
...
function extractCitations(context) {
  ...
  const page =
    context && typeof context.page === "object" ? context.page : null;
  if (page && typeof page.url === "string" && page.url.trim()) {
    const title =
      sanitizeString(page.title || "Page context", 140) || "Page context";
  ...
  const profile =
    context && typeof context.profile === "object" ? context.profile : null;
  ...
  links.slice(0, 6).forEach((link) => {
    ...
    citations.push({
      title:
        sanitizeString(link.label || "Profile link", 140) || "Profile link",
      url: href,
      snippet: "",
      sourceType: "profile",
    });
...
function buildPrompt(query, source, context) {
  ...
  if (context && typeof context === "object") {
    ...
    if (profile) {
      ...
      if (profile.role)
        parts.push(`- Role: ${sanitizeString(profile.role, 120)}`);
      if (profile.summary)
        parts.push(`- Summary: ${sanitizeString(profile.summary, 600)}`);
...
async function verifyAuth(request, env) {
  ...
  const role =
    sanitizeString(payload.role || "user", 40).toLowerCase() || "user";
...
async function chargeCredits(request, env, amount) {
  ...
  if (!chargeUrl) {
    throw new HttpError(
      ...
      "Credit charging is not configured",
      500,
    );
  }
  ...
  if (response.status === 402) {
    throw new HttpError(
      ...
      "Not enough credits to use Ask AI",
      402,
    );
  }
...
async function runGemini(env, prompt, history) {
  ...
  const maxOutputTokens = clamp(
    env.GEMINI_MAX_OUTPUT_TOKENS || DEFAULT_MAX_OUTPUT_TOKENS,
    128,
    2048,
  );
  ...
  if (!response.ok) {
    ...
    throw new HttpError(
      ...
      `Gemini request failed: ${text.slice(0, 180)}`,
      502,
    );
  }
...
async function handleAsk(request, env, origin) {
  ...
  const cost =
    role === "admin"
      ? 0
      : clamp(env.CREDIT_COST || DEFAULT_CREDIT_COST, 0, 1000);
...
export default {
  async fetch(request, env) {
    ...
    if (
      request.method !== "POST" ||
      (url.pathname !== "/v1/ask" && url.pathname !== "/ask")
    ) {
      return jsonResponse(
